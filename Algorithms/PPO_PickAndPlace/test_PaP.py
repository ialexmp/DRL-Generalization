...
from Networks_PaP import ActorNetwork, CriticNetwork
    ...
    for _ in range(num_episodes):
        ...
        while not done:
            ...
            with torch.no_grad():
                ...
                action = action_distribution.mean.detach().numpy().squeeze(axis=0)       
...
def main(experiment_name):
    ...
    # Create environment
    custom_env_name = 'CustomFetchPickAndPlace'
    render = True
    if render:
        env = gym.make(custom_env_name, render_mode='human')
    else: 
        env = gym.make(custom_env_name)

    # Get state and action space sizes
    observation_size = env.observation_space["observation"].shape[0] + \
