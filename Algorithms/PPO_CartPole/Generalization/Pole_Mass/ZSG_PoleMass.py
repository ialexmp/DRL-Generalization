...
from Custom_Env.custom_envs import CustomCartPoleMassEnv
...
def plot_results(parameter_values, avg_rewards, std_rewards, trained_value, parameter_name):
    ...
    plt.title('Modified CartPole Pole Mass')
    plt.legend()
    
    # Save the plot in the current directory
    current_dir = os.path.dirname(os.path.abspath(__file__))
    plot_path = os.path.join(current_dir, 'ZSG_PoleMass_Results.png')
...
def main(experiment_name):
    ...
    parameter_name = 'Pole Mass'
    trained_value = 0.1  # The value where the agent was trained
    parameter_values = np.arange(0.0, 0.6, 0.05) 
    render = False

    env = gym.make('CustomCartPoleMass', masspole=trained_value)
    ...
    for value in parameter_values:
        if render: 
            custom_env = gym.make('CustomCartPoleMass', masspole=value, render_mode='human')
        else:
            custom_env = gym.make('CustomCartPoleMass', masspole=value)
        
        print(f"Testing with Mass: {value}")
