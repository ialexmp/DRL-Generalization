...
from Custom_Env.custom_envs import CustomCartPoleGravityEnv
...
def plot_results(parameter_values, avg_rewards, std_rewards, trained_value, parameter_name):
    ...
    plt.title('Modified CartPole Pole Gravity')
    plt.legend()
    
    # Save the plot in the current directory
    current_dir = os.path.dirname(os.path.abspath(__file__))
    plot_path = os.path.join(current_dir, 'ZSG_Gravity_Results.png')
...
def main(experiment_name):
    ...
    trained_value = 9.8  # The value where the agent was trained
    parameter_values = np.arange(5, 15, 1) 
    render = False

    env = gym.make('CustomCartPoleGravity', gravity=trained_value)
    ...
    for value in parameter_values:
        if render: 
            custom_env = gym.make('CustomCartPoleGravity', gravity=value, render_mode='human')
        else:
            custom_env = gym.make('CustomCartPoleGravity', gravity=value)
        
        print(f"Testing with Gravity: {value}")
