...
from Custom_Env.custom_envs import CustomCartPoleLengthEnv
...
def plot_results(parameter_values, avg_rewards, std_rewards, trained_value, parameter_name):
    ...
    plt.title('Modified CartPole Pole Length')
    plt.legend()
    
    # Save the plot in the current directory
    current_dir = os.path.dirname(os.path.abspath(__file__))
    plot_path = os.path.join(current_dir, 'ZSG_PoleLength_Results.png')
...
def main(experiment_name):
    ...
    parameter_name = 'Pole Length'
    trained_value = 0.5  # The value where the agent was trained
    parameter_values = np.arange(0.2, 0.8, 0.1) 
    render = False

    env = gym.make('CustomCartPoleLength', lenght=trained_value)
    ...
    for value in parameter_values:
        if render: 
            custom_env = gym.make('CustomCartPoleLength', lenght=value, render_mode='human')
        else:
            custom_env = gym.make('CustomCartPoleLength', lenght=value)
        
        print(f"Testing with Lenght: {value}")
        avg_reward, std_reward = test_actor_model(actor, custom_env, num_episodes=10)
