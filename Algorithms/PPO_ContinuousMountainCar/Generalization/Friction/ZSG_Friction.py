...
from Custom_Env.custom_envs import CustomMountainCarFrictionEnv
...
def plot_results(parameter_values, avg_rewards, std_rewards, trained_value, parameter_name):
    ...
    plt.title('Modified Continuous Mountain Car Friction')
    plt.legend()
    ...
    plot_path = os.path.join(current_dir, 'ZSG_CarFriction_Results.png')
...
def main(experiment_name):
    ...
    parameter_name = 'Friction'
    trained_value = 0.0025  # The value where the agent was trained
    parameter_values = np.arange(0.001, 0.0065, 0.0005)  
    render = False
    ...
    for value in parameter_values:
        if render: 
            custom_env = gym.make('CustomMountainCarFriction', car_friction=value, render_mode='human')
        else:
            custom_env = gym.make('CustomMountainCarFriction', car_friction=value)
        
        print(f"Testing with Friction: {value}")
