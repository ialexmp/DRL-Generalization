...
from Custom_Env.custom_envs import CustomMountainCarInitialPositionEnv
...
def plot_results(parameter_values, avg_rewards, std_rewards, trained_value_low, trained_value_high, parameter_name):
    ...
    plt.axvline(x=trained_value_low, color='red', linestyle='--', linewidth=1, label=f'Trained at range [{trained_value_low}, {trained_value_high}]')
    plt.axvline(x=trained_value_high, color='red', linestyle='--', linewidth=1)

    plt.xlabel(parameter_name)
    plt.ylabel('Average Reward')
    plt.title('Modified Continuous Mountain Car Initial Position')
    plt.legend()
    
    # Save the plot in the current directory
    current_dir = os.path.dirname(os.path.abspath(__file__))
    plot_path = os.path.join(current_dir, 'ZSG_CarInitialPosition_Results.png')
...
def main(experiment_name):
    ...
    parameter_name = 'Car Initial Position'
    trained_value_high = -0.04  # The value where the agent was trained
    trained_value_low = -0.06  # The value where the agent was trained
    parameter_values = np.arange(-0.6, 0.3, 0.1)  
    render = False
    ...
    for value in parameter_values:
        if render: 
            custom_env = gym.make('CustomMountainCarInitialPosition', initial_pos=value, render_mode='human')
        else:
            custom_env = gym.make('CustomMountainCarInitialPosition', initial_pos=value)
        
        print(f"Testing with Intial Position: {value}")
        avg_reward, std_reward = test_actor_model(actor, custom_env, num_episodes=500)
        avg_rewards.append(avg_reward)
        std_rewards.append(std_reward)
        custom_env.close()

    plot_results(parameter_values, avg_rewards, std_rewards, trained_value_low, trained_value_high, parameter_name)
