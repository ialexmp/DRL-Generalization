...
from Custom_Env.custom_envs import CustomMountainCarGoalPositionEnv
...
def plot_results(parameter_values, rewards, trained_value, parameter_name):
    ...
    plt.title('PPO Generalization Across Modified Goal Positions')
    plt.legend()
    
    # Save the plot in the current directory
    current_dir = os.path.dirname(os.path.abspath(__file__))
    plot_path = os.path.join(current_dir, 'ZSG_GoalPositions_Results.png')
...
def main(experiment_name):
    ...
    parameter_name = 'Goal Position'
    trained_value = 0.45  # The value where the agent was trained
    parameter_values = np.arange(0.25, 0.65, 0.05)  # Range from 0.25 to 0.6 with step of 0.05
    render = False
    ...
    for value in parameter_values:
        if render: 
            custom_env = gym.make('CustomMountainCarGoalPosition', goal_position=value, render_mode='human')
        else:
            custom_env = gym.make('CustomMountainCarGoalPosition', goal_position=value)
        
        avg_reward = test_actor_model(actor, custom_env, num_episodes=500)
