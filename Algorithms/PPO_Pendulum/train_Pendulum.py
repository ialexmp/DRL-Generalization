...
from trainer_Pendulum import Trainer
...
def run_experiment(exp_name, experiment_id, config, device):
    ...
    # Create environment
    custom_env_name = 'Pendulum-v1'
    
    render = False
    if render:
        env = gym.make(custom_env_name, g=9.81, render_mode='human')
    else: 
        env = gym.make(custom_env_name, g=9.81)
