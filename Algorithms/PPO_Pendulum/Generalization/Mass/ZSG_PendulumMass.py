...
from Custom_Env.custom_envs import CustomPendulumMassEnv
...
def plot_results(parameter_values, avg_rewards, std_rewards, trained_value, parameter_name):
    ...
    plt.title('Modified Pendulum Mass')
    plt.legend()
    
    # Save the plot in the current directory
    current_dir = os.path.dirname(os.path.abspath(__file__))
    plot_path = os.path.join(current_dir, 'ZSG_PendulumMass_Results.png')
...
def main(experiment_name):
    ...
    parameter_values = np.arange(0.25, 2.5, 0.25)  
    render = False
    ...
    for value in parameter_values:
        if render: 
            custom_env = gym.make('CustomPendulumMass', m=value, render_mode='human')
        else:
            custom_env = gym.make('CustomPendulumMass', m=value)
