...
from Custom_Env.custom_envs import CustomPendulumDeltaTimeEnv
...
def plot_results(parameter_values, avg_rewards, std_rewards, trained_value, parameter_name):
    ...
    plt.title('Modified Pendulum Delta Time')
    plt.legend()
    
    # Save the plot in the current directory
    current_dir = os.path.dirname(os.path.abspath(__file__))
    plot_path = os.path.join(current_dir, 'ZSG_PendulumDeltaTime_Results.png')
...
def main(experiment_name):
    ...
    parameter_name = 'Delta Time'
    trained_value = 0.05 # The value where the agent was trained
    parameter_values = np.arange(0.01, 0.15, 0.01)  
    render = False
    ...
    for value in parameter_values:
        if render: 
            custom_env = gym.make('CustomPendulumDeltaTime', dt=value, render_mode='human')
        else:
            custom_env = gym.make('CustomPendulumDeltaTime', dt=value)
        
        print(f"Testing with DeltaTime: {value}")
