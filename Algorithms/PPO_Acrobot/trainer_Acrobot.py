...
from Network_Acrobot import ActorNetwork, CriticNetwork
...
class Trainer:
    def __init__(
        ...
        ε: float = 0.2,
        α: float = 3e-4,
        training_cycles_per_batch: int = 10,
        save_every_x_timesteps: int = 1_000_000,
        device = 'cuda'
    ):  
        self.device = device
    ...
    def run_episode(self) -> EpisodeMemory: # type: ignore
        ...
        while True:
            ...
            log_probability = action_distribution.log_prob(action).item()

            # Convert action to an integer
            action = action.item()

            next_state, reward, terminated, _, _ = self.env.step(action)
            observation = torch.tensor(next_state, dtype=torch.float32).to(self.device)

            actions.append(action)
            log_probabilities.append(log_probability)
            rewards.append(reward)

            if timesteps >= self.max_timesteps_per_episode or terminated:
                break
