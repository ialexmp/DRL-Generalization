...
from Custom_Env.custom_envs import ModifiedAcrobotCoMPositionsEnv
...
def plot_results(parameter_values, rewards_link1, rewards_link2, std_link1, std_link2, trained_value, parameter_name):
    ...
    plt.axvline(x=trained_value, color='red', linestyle='--', linewidth=1, label=f'Trained CoM at {trained_value}')
    
    plt.xlabel(parameter_name)
    plt.ylabel('Average Reward')
    plt.title('Modified Acrobot Center Of Mass (CoM) Positions')
    plt.legend()
    
    current_dir = os.path.dirname(os.path.abspath(__file__))
    plot_path = os.path.join(current_dir, 'ZSG_CoM_Results.png')
...
def main(experiment_name):
    ...
    trained_value = 0.5
    parameter_values = np.arange(0.1, 3.0, 0.2)
    render = False
    ...
    for value in parameter_values:
        if render:
            custom_env_link1 = gym.make('ModifiedAcrobotCoMPositions', com_pos_1=value, com_pos_2=trained_value, render_mode='human')
            custom_env_link2 = gym.make('ModifiedAcrobotCoMPositions', com_pos_1=trained_value, com_pos_2=value, render_mode='human')
        else:
            custom_env_link1 = gym.make('ModifiedAcrobotCoMPositions', com_pos_1=value, com_pos_2=trained_value)
            custom_env_link2 = gym.make('ModifiedAcrobotCoMPositions', com_pos_1=trained_value, com_pos_2=value)
        ...
        avg_reward_link2, std_reward_link2 = test_actor_model(actor, custom_env_link2, num_episodes=200)
        avg_rewards_link2.append(avg_reward_link2)
        std_rewards_link2.append(std_reward_link2)
        custom_env_link2.close()

    plot_results(parameter_values, avg_rewards_link1, avg_rewards_link2, std_rewards_link1, std_rewards_link2, trained_value, 'Center of Mass Position')
