...
from Custom_Env.custom_envs import ModifiedAcrobotLinkMassesEnv
...
def plot_results(parameter_values, rewards_link1, rewards_link2, std_link1, std_link2, trained_value, parameter_name):
    ...
    plt.axvline(x=trained_value, color='red', linestyle='--', linewidth=1, label=f'Trained Mass at {trained_value}')
    
    plt.xlabel(parameter_name)
    plt.ylabel('Average Reward')
    plt.title('Modified Acrobot Mass')
    plt.legend()
    
    current_dir = os.path.dirname(os.path.abspath(__file__))
    plot_path = os.path.join(current_dir, 'ZSG_Mas_Results.png')
...
def main(experiment_name):
    ...
    trained_value = 1.0
    parameter_values = np.arange(0.25, 4.5, 0.25) 
    render = False
    ...
    for value in parameter_values:
        if render:
            custom_env_link1 = gym.make('ModifiedAcrobotLinkMasses', link_mass_1=value, link_mass_2=trained_value, render_mode='human')
            custom_env_link2 = gym.make('ModifiedAcrobotLinkMasses', link_mass_1=trained_value, link_mass_2=value, render_mode='human')
        else:
            custom_env_link1 = gym.make('ModifiedAcrobotLinkMasses', link_mass_1=value, link_mass_2=trained_value)
            custom_env_link2 = gym.make('ModifiedAcrobotLinkMasses', link_mass_1=trained_value, link_mass_2=value)
